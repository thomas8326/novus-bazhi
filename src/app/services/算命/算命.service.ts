...
export class 算命服務器 {
  ...
  private 流年剋大運(對象命盤: 天干命盤 | 地支命盤) {
    if (!對象命盤.命盤結果.大運已作用 && !對象命盤.命盤結果.流年已作用 && this.是否相刻(對象命盤.流年, 對象命盤.大運)) {
      對象命盤.命盤結果.大運已作用 = true;
  ...
  private 大運剋流年(對象命盤: 天干命盤 | 地支命盤) {
    if (!對象命盤.命盤結果.大運已作用 && !對象命盤.命盤結果.流年已作用 && this.是否相刻(對象命盤.大運, 對象命盤.流年)) {
  ...
  private 本命大運流年合(對象命盤: 天干命盤 | 地支命盤) {
    for (let i = 0; i < 對象命盤.本命.length; i++) {
      if (this.是否為天干 && i === 1) {
        // 日住不作用
        continue;
  ...
  private 解合(對象命盤: 天干命盤 | 地支命盤) {
    ...
    for (let i = 0; i < 對象命盤.本命.length; i++) {
      ...
      if (!流年是否有相生) {
        流年是否有相生 = this.是否相生(對象命盤.流年, 對象命盤.本命[i]);
      }

      if (
        對象命盤.命盤結果.大運已作用 &&
        對象命盤.命盤結果[this.年月日時住轉換(i)] &&
        this.是否相合(對象命盤.本命[i], 對象命盤.大運)
      ) {
        大運本命相合索引 = i;
      }
    }

    if (!流年是否有相生 && 大運本命相合索引 >= 0) {
      對象命盤.命盤結果[this.年月日時住轉換(大運本命相合索引)] = false;
      對象命盤.命盤結果.大運已作用 = false;
      return true;
    }
  ...
  private 是否相生(作用主體: 天干 | 地支, 被作用目標: 天干 | 地支) {
    ...
    return 五行相生對照表.get(作用主體五行) === 被作用目標五行;
  }
